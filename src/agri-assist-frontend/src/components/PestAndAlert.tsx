...
const agriculturalAlerts: PestDiseaseAlert[] = [
  {
    ...
    description:
      "Devastating oomycete causing rapid plant destruction. Spreads through water-splashed spores.",
    prevention: [
    ...
    lifecycle: [
      "Spore germination (4h)",
      "Leaf penetration (24h)",
      "Sporulation (3-5 days)",
    ],
  },
  {
    id: "faw",
    ...
    description:
      "Polyphagous lepidopteran pest with 8-9 annual generations. Causes 20-50% yield loss.",
    prevention: [
...
const PestDiseaseAlerts: React.FC = () => {
  ...
  const [severityFilter, setSeverityFilter] = useState<
    "" | PestDiseaseAlert["severity"]
  >("");

  const filterAlerts = useCallback(() => {
    return agriculturalAlerts.filter(
      (alert) =>
        alert.affectedCrops.some(
          (c) => c.toLowerCase() === selectedCrop.toLowerCase()
        ) &&
        alert.affectedRegions.some(
    ...
    <div className="p-6 max-w-6xl mx-auto bg-gray-50 min-h-screen">
      <div className="mb-8 flex items-center gap-4">
        ...
        <h1 className="text-2xl font-semibold text-gray-800">
          Agricultural Threat Management System
        </h1>
      </div>
      ...
      <div className="grid grid-cols-3 gap-4 mb-8 bg-white p-4 rounded-lg shadow-sm border">
        <div>
          ...
          >
            {Array.from(
              new Set(agriculturalAlerts.flatMap((a) => a.affectedCrops))
            ).map((crop) => (
        ...
        <div>
          ...
          >
            {Array.from(
              new Set(agriculturalAlerts.flatMap((a) => a.affectedRegions))
            ).map((region) => (
